...
import Footer from "@/app/(root)/component/footer/footer"
import Header from "@/app/(root)/component/header/header"
...
export default function BookingSection() {
  ...
  const fetchAllServicesAndSubServices = useCallback(
    async (showRefreshIndicator = false) => {
      try {
        ...
        if (formData.serviceId && !activeServices.find((s: Service) => s._id === formData.serviceId)) {
          setFormData((prev) => ({ ...prev, serviceId: "", subServiceId: "", selectedType: "service" }))
        }
        if (
          formData.subServiceId &&
          !flattenedSubServices.find((ss: SubServiceWithService) => ss._id === formData.subServiceId)
        ) {
          setFormData((prev) => ({ ...prev, subServiceId: "", selectedType: "service" }))
        }
      } catch (error) {
  ...
  const handleSubServiceSelect = (subServiceId: string) => {
    const selectedSubService = allSubServices.find((ss) => ss._id === subServiceId)
    setFormData({
      ...formData,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.selectedType === "subservice" && formData.subServiceId) {
        selectedSubService = allSubServices.find((s) => s._id === formData.subServiceId)
        selectedService = services.find((s) => s._id === selectedSubService?.serviceId)
      ...
      setSubmitMessage(
        `Thank you ${formData.fullName.split(" ")[0]}! Your request has been submitted successfully. We'll contact you within 24 hours. Your reference ID is: ${result.bookingId || bookingId}`,
      )
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-8 sm:mb-12">
            ...
            <p className="text-lg sm:text-xl text-gray-600 max-w-2xl mx-auto">
              Ready to get started? Choose from our services or specific offerings below, then we'll get back to you
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="space-y-8">
                <div className="text-center mb-6">
                  <h3 className="text-xl sm:text-2xl font-bold text-gray-800 mb-2">Contact Information</h3>
                ...
                <div>
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      <h3 className="text-xl sm:text-2xl font-bold text-gray-800">Choose Your Service</h3>
                      ...
                      {formData.serviceId && (
                        <div className="mb-8">
                          <div className="flex items-center mb-4">
                            ...
                            <div>
                              <h4 className="text-lg sm:text-xl font-bold text-gray-800">Choose Specific Service</h4>
                          ...
                          {isLoadingServices ? (
                            <div className="text-center py-8">
                              ...
                              <p className="mt-2 text-gray-600">Loading specific services...</p>
                            </div>
                          ) : (
                            <>
                              {/* Sub-Service Cards */}
                              {allSubServices.filter((ss) => ss.serviceId === formData.serviceId).length > 0 ? (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                                  {allSubServices
                                    .filter((ss) => ss.serviceId === formData.serviceId)
                                    .map((subService) => (
                                      <div
                                        key={subService._id}
                                        onClick={() => handleSubServiceSelect(subService._id)}
                                        className={`cursor-pointer p-5 rounded-xl border-2 transition-all duration-300 hover:shadow-lg transform hover:-translate-y-1 ${
                                          formData.subServiceId === subService._id
                                          ...
                                          <div className="flex items-center">
                                            <span className="text-xs text-gray-400">
                                              {subService.priceType === "fixed"
                                                ? "One-time"
                                                : subService.priceType === "hourly"
                                                  ? "Per hour"
                                                  : subService.priceType === "per_item"
                                                    ? "Per item"
                                                    : "Custom"}
                                            </span>
                                          </div>
                                ...
                                <div className="text-center py-8 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                                  ...
                                  <p className="text-gray-500 text-lg font-medium">No specific services available</p>
                                  <p className="text-gray-400 text-sm mt-1">
                                    No sub-services have been configured for this service yet.
                                  </p>
                                </div>
                              )}
                            </>
                      ...
                      {formData.serviceId && (
                        <div className="mb-8">
                          <div className="flex items-center mb-4">
                            ...
                            <div>
                              <h4 className="text-lg sm:text-xl font-bold text-gray-800">Write in Details</h4>
                              <p className="text-sm text-gray-600">
                                Please describe your specific needs, requirements, or any special instructions for the
                                admin
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-xl p-6 border border-purple-200">
                            <label htmlFor="details" className="block text-sm font-medium text-gray-700 mb-3">
...
• Any other important details the admin should know..."
                            ...
                            <div className="mt-2 text-xs text-gray-500">
                              <span className="font-medium">Tip:</span> The more details you provide, the more accurate
                              quote we can give you.
                            </div>
                          </div>
                        </div>
                      )}
                      ...
                      {services.length === 0 && (
                        <div className="text-center py-12">
                          ...
                          <p className="text-gray-400 text-sm mt-2">Please contact us directly for assistance.</p>
                        </div>
                      )}
                    </>
                  )}
